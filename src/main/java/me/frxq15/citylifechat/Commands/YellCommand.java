...
public class YellCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(!p.hasPermission("clchat.yell")) {
            p.sendMessage(CityLifeChat.formatMsg("NO_PERMISSION"));
            return true;
        }
        if(args.length == 0) {
            p.sendMessage(CityLifeChat.colourize("&cUsage: /yell <text>"));
        }
        final String message = this.getFinalArg(args, 0);
        RadiusManager.getNearbyPlayers(p, CityLifeChat.getInstance().getConfig().getDouble("YELL_CHAT_RADIUS")).forEach(r -> {
            r.sendMessage(CityLifeChat.formatMsg("YELL_CHAT_FORMAT").replace("%player%", p.getName()).replace("%chat%", message));
        });
        p.sendMessage(CityLifeChat.formatMsg("YELL_CHAT_FORMAT").replace("%player%", p.getName()).replace("%chat%", message));
        return true;
    }
