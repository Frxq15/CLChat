...
public class TwitterCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if(!p.hasPermission("clchat.twitter")) {
            p.sendMessage(CityLifeChat.formatMsg("NO_PERMISSION"));
            return true;
        }
        if(args.length == 0) {
            p.sendMessage(CityLifeChat.colourize("&cUsage: /twitter <text>"));
        }
        final String message = this.getFinalArg(args, 0);
        String pmessage = PlaceholderAPI.setPlaceholders(p, CityLifeChat.formatMsg("TWITTER_CHAT_FORMAT").replace("%player%", p.getName()).replace("%chat%", message));
